...
public class Camera {
    ...
    public void Yaw(float dYaw) { // rotate v , n->u
        ...
        nAxis[0] = sina*uAxis[0]+cosa*nAxis[0];
        nAxis[1] = sina*uAxis[1]+cosa*nAxis[1];
        nAxis[2] = sina*uAxis[2]+cosa*nAxis[2];
    ...
    public void Roll(float dRoll) { // rotate n , u->v
        ...
        vAxis[0] = sina*uAxis[0]+cosa*vAxis[0];
        vAxis[1] = sina*uAxis[1]+cosa*vAxis[1];
        vAxis[2] = sina*uAxis[2]+cosa*vAxis[2];
    ...
    public void Zoom(float delta) {
        if(fovy_MIN<fovy&&fovy<fovy_MAX) {
            fovy += 5 * DELTA_CONSTANT * delta;
        }
        else{
            if(fovy<=fovy_MIN)
                fovy=fovy_MIN+1;
            else if(fovy>=fovy_MAX)
                fovy=fovy_MAX-1;
        }
    }
